...
export const generateColumnsBindedAnkets = (
  data,
  removeAction = null,
  editAction = null,
  sourceId,
    ...
    ?.map((key) => {
            ...
            return (
              ...
              >
                <Button
                  ...
                  text="Просмотреть"
                  func={(e) =>
                    editAction(e, original.id, sourceId, sourceNameId)
                  }
                />
                <Button
                  mode="tretiary"
                  text="Исключить"
                  func={(e) => removeAction(e, sourceId, original.id)}
                />
              </div>
            );
          },
          ...
          Cell: ({ row: { original } }) => {
            return (
              <div className="user_column in_folder_user">
                ...
                <p>
                  {original.firstname || "-"} {original.patronymic || "-"}
                </p>
              </div>
            );
          },
      ...
      if (key === "dob") {
        return {
          ...
          className: "tableColumn",
          Cell: ({ row: { original } }) => {
            const age = moment().diff(`${original.dob}`, "years");
            return (
              <div className="user_column">
                <p>{moment(original.dob).format("YYYY-MM-DD") || "-"}</p>
                <p>{age || "-"} лет</p>
              </div>
            );
          },
      ...
      if (key === "birthdate") {
        return {
          ...
          className: "tableColumn",
          Cell: ({ row: { original } }) => {
            const age = moment().diff(`${original.birthdate}`, "years");
            return (
              <div className="user_column">
                <p>{moment(original.birthdate).format("YYYY-MM-DD") || "-"}</p>
                <p>{age || "-"} лет</p>
              </div>
            );
          },
